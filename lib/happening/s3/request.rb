  ...
  module S3
    class Request
      ...
      def initialize(http_method, url, options = {})
        ...
        assert_valid_keys(options, :timeout, :on_success, :on_error, :retry_count, :headers, :data, :ssl, :proxy)
      ...
      def execute
        ...
        @response = http_class.new(url,:proxy => options[:proxy]).send(http_method, :timeout => options[:timeout], :head => options[:headers], :body => options[:data], :ssl => options[:ssl])
